...
class SegImageDataLayer(caffe.Layer):
    ...
    def transform(self):
        ...
        if (self.new_height>0) or (self.new_width>0):
            ...
            for i in range(self.batch_size):
                new_data = cv2.resize(self.data_buf[i].transpose((1,2,0)),(rs_width,rs_height),cv2.INTER_NEAREST)
                if len(new_data.shape) == 2:
                    new_data = new_data[np.newaxis,...]
                else:
                    new_data = new_data.transpose((2,0,1))
                self.data_buf[i] = new_data
                new_label = cv2.resize(self.label_buf[i].transpose((1,2,0)),(rs_width,rs_height),cv2.INTER_NEAREST)
